...
Resolution order (first one that finds a real dimension wins):
    ...
    b) a real column name mentioned in the text
    c) a business-word synonym ("stores" -> "store", "sellers" -> "seller")
    d) a ranking request ("top ...") on a table with exactly ONE groupable
       column -> that is the only honest reading
    e) the LLM fallback

The measure is resolved the same way, plus: "selling"/"revenue"/"money" signal
a sales intent without naming the column, and a ranking request implies
ordering by the main measure rather than counting rows.
...
_DIMENSION_SYNONYMS: tuple[tuple[re.Pattern[str], tuple[str, ...]], ...] = (
    ...
    (
        re.compile(r"\bholidays?\b|\bfestivos?\b", re.IGNORECASE),
        ("holiday", "festivo"),
    ),
    (
...
def _confirmation(request: ChartRequest) -> str:
    ...
    return (
        f"Added: {what} by {scope}**{request.dimension}**, "
...
- Use agg="mean" when the user asks for an average, "sum" for totals.
- If there is no clear measure, use measure=null and agg="count".
- If the request cannot be answered with these columns, return {{"dimension": null}}.
...
def interpret_message(text: str, df: pd.DataFrame) -> ChatChartResult:
    ...
    if dimension is None:
        ...
        if proposal is not None:
            return ChatChartResult(proposal, _confirmation(proposal))
    ...
    if measure is None and measure_priority:
        ...
        # sales/money, or asks for a ranking ("top 5 stores"): in a ranking,
        # counting rows is almost never what was meant.
        if _SELLING_INTENT.search(message) or ranking_intent:
