...
def build_planning_crew(
    ...
    task = Task(
        description=(
            ...
            "When inspect_profile reports CANDIDATE_NUMERIC_TEXT_NOISE for a "
            "column, that column is numeric text carrying symbols: propose "
            "propose_normalize_numeric_text for it and then propose_cast_column "
            "to NUMERIC, in that order, as two operations citing that same "
            "issue. Do this without being asked; both still go to the human for "
            "approval like any other proposal. "
            "Use the allow-listed proposal tools whenever they can express the "
            "objective. Imputation and column dropping are supported: use "
            "propose_impute_missing and propose_drop_column. Only call "
            "report_unsupported_request when the requested behavior truly cannot "
            "be represented by an available operation. "
            "For an explicit binary arithmetic request, use "
            "propose_compute_column with two numeric columns, one new output "
            "column, and one of ADD, SUBTRACT, MULTIPLY, or DIVIDE; never invent "
            "an expression string. "
            "Missing values: use the requested imputation strategy directly. "
            "For conditional drop/imputation requests, inspect the deterministic "
            "profile before choosing the operation. "
            "Use the deterministic column_statistics returned by inspect_profile "
            "for conditional decisions. null_count is the exact number of nulls; "
            "zero_count is the exact number of zero-valued cells. Never request or "
            "assume access to cell values. "
            "When the objective supplies a missingness threshold such as X.X%, "
            "compare the exact null_count with row_count rather than relying on "
            "diagnostic issue presence or rounded percentages. "
